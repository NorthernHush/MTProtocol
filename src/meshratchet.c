...
mr_ctx_t* mr_init_ex(const mr_config_t* config) {
    ...
    log_message(ctx, MR_LOG_INFO, "MeshRatchet v%s initialized", MESHRATCHET_VERSION);
    return ctx;
}
...
int mr_session_create(mr_ctx_t* ctx, const mr_key_pair_t* local_key,
    ...
    *session = sess;
    ...
    log_message(ctx, MR_LOG_INFO, "New session created (ID: %02x%02x...)", 
                sess->session_id[0], sess->session_id[1]);
    return MR_SUCCESS;
}
...
int mr_encrypt(mr_session_t* session, mr_msg_type_t msg_type,
    ...
    *ct_len = offset + written;
    ...
    log_message(session->ctx, MR_LOG_DEBUG, "Message encrypted (type: %d, seq: %lu, size: %zu)", 
                msg_type, sequence, *ct_len);
    return MR_SUCCESS;
}
...
int mr_decrypt(mr_session_t* session, 
    ...
    *msg_type = (mr_msg_type_t)ciphertext[offset++];
    ...
    if (!ok) {
        log_message(session->ctx, MR_LOG_ERROR, "Decryption failed (seq: %lu)", sequence);
        return MR_ERROR_CRYPTO;
    }

    *pt_len = written;
    
    log_message(session->ctx, MR_LOG_DEBUG, "Message decrypted (type: %d, seq: %lu, size: %zu)", 
                *msg_type, sequence, *pt_len);
    return MR_SUCCESS;
}
...
int mr_key_update(mr_session_t* session) {
    ...
    log_message(session->ctx, MR_LOG_INFO, "Key update performed (count: %u)", session->key_update_count);
    return MR_SUCCESS;
}
...
int mr_emergency_key_update(mr_session_t* session) {
    ...
    log_message(session->ctx, MR_LOG_WARN, "Emergency key update triggered");
    
    // Генерация completely новых ключей
    if (generate_random(session->ctx, session->send_ratchet_priv, 32) != MR_SUCCESS) {
        return MR_ERROR_CRYPTO;
    }
    
    // Сброс всех состояний
    if (generate_random(session->ctx, session->root_key, MR_ROOT_KEY_LEN) != MR_SUCCESS) {
        return MR_ERROR_CRYPTO;
    }
    ...
    log_message(session->ctx, MR_LOG_INFO, "Emergency key update completed");
    return MR_SUCCESS;
}
...
int mr_get_session_info(const mr_session_t* session, mr_session_info_t* info) {
    ...
    info->is_active = (time(NULL) - session->last_activity < 3600); // 1 hour timeout
...
void mr_session_free(mr_session_t* session) {
    if (session) {
        log_message(session->ctx, MR_LOG_DEBUG, "Session freed (encrypted: %u, decrypted: %u)", 
...
int mr_get_library_info(char* buffer, size_t buffer_len) {
    const char* info = "MeshRatchet Protocol v" MESHRATCHET_VERSION "\n"
                      ...
                      "Algorithms: X25519, HKDF, ChaCha20-Poly1305";
